...
#[cfg(windows)]
fn as_key(path:&str) -> Cow<str> { Cow::Owned(path.replace("\\", "/")) }
...
impl IncludeDir {
	...
	/// For chaining, it's not sensible to return a Result. If any
	/// to-be-included files can't be found, or encoded, this function will
	/// panic!.
	...
	pub fn add_file<P:AsRef<Path>>(
		...
		match comp {
			...
			Compression::Gzip => {
				...
				let out_path =
					Path::new(&env::var("OUT_DIR").unwrap()).join(&path);
				fs::create_dir_all(&out_path.parent().unwrap())?;
				let out_file = BufWriter::new(File::create(&out_path)?);
				let mut encoder =
					GzEncoder::new(out_file, flate2::Compression::best());
	...
	pub fn build(self, out_name:&str, filter:Vec<String>) -> io::Result<()> {
		...
		writeln!(
			&mut out_file,
			"pub static {}: ::tauri_includedir::Files = \
			 ::tauri_includedir::Files {{\nfiles:  ",
			self.name
		)?;
		...
		let entries:Vec<_> = self
			...
			.map(|(name, (compression, path))| {
				let include_path =
					format!("{}", self.manifest_dir.join(path).display());
				(
		...
		for (name, (compression, include_path)) in &entries {
			if !filter.iter().any(|value| include_path.ends_with(value)) {
				let include_path = format!(
					"{}",
					self.manifest_dir.join(include_path).display()
				);
				map.entry(
					name,
					&format!(
